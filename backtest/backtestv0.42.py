...
def main():
    ...
    while chart_data_list[0].time < btc.end_time:
        ...
        is_upward_pressure = isUpwardPressure(compass_time_frame_data)
        is_downward_pressure = isDownwardPressure(compass_time_frame_data)

        if is_upward_pressure is None or is_downward_pressure is None:
            if pos != "N":
                if pos == "S":
                    if pos_price != -1:
        ...
        if is_upward_pressure and isGoodLongEntry(entry_time_frame_data):
            if pos != "L":
                if pos_price != -1:
                    profit_percent += (pos_price - price) / pos_price
                    # max draw
        ...
        elif is_downward_pressure and isGoodShortEntry(entry_time_frame_data):
            if pos != "S":
                if pos_price != -1:
                    profit_percent += (price - pos_price) / pos_price
                    # max draw
