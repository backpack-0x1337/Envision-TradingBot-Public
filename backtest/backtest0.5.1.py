...
def main():
    btc = Backtest(1610974080, 1615639680, getPathArray(), 0, 0)
    ...
    while chart_data_list[0].time < btc.end_time:
        ...
        if isUpwardPressure(compass_time_frame_data) or isUpwardPressure(entry_time_frame_data):
            if pos != "L":
                if pos_price != -1:
        ...
        elif isDownwardPressure(compass_time_frame_data) or isDownwardPressure(entry_time_frame_data):
            if pos != "S" :
                if pos_price != -1:
