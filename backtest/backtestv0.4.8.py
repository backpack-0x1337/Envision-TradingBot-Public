...
def isUpwardPressure(compass_time_frame_data):
    ...
    for tf_data in compass_time_frame_data:
        if tf_data.red >= 56 and tf_data.white >= 56:
            upward_tf = counter
        if tf_data.red <= 48 and tf_data.white <= 48:
            downward_tf = counter
        counter += 1
...
def main():
    ...
    while chart_data_list[0].time < btc.end_time:
        ...
        if isUpwardPressure(compass_time_frame_data) and isGoodLongEntry(entry_time_frame_data):
            if pos != "L":
                if pos_price != -1:
                    ...
                    # max draw
                    if profit_percent > 0 and profit_percent > max_up:
                        max_up = profit_percent
                    elif profit_percent < 0 and profit_percent < max_down:
                        max_down = profit_percent
                    init_invest = init_invest * (1 - fee)
                    init_invest += init_invest * (profit_percent * leverage)
                    print("tradeNo = {} LastPos = {}, profit = {}, Account balance = {}, UnixTime = {}"
                          .format(tradeNo, pos, profit_percent, init_invest, chart_data_list[0].time))
        ...
        elif isDownwardPressure(compass_time_frame_data) and isGoodShortEntry(entry_time_frame_data):
            if pos != "S" :
                if pos_price != -1:
