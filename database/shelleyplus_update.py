...
def get_entry_list(TimeFrameData, pair):
    entry_tf_list = [
        ...
        TimeFrameData.query.filter_by(pair=pair, timeframe='24').first(),
        TimeFrameData.query.filter_by(pair=pair, timeframe='48').first()
    ]
...
def get_compass_list(TimeFrameData, pair):
    compass_tf_list = [
        TimeFrameData.query.filter_by(pair=pair, timeframe='96').first(),
        TimeFrameData.query.filter_by(pair=pair, timeframe='192').first(),
        TimeFrameData.query.filter_by(pair=pair, timeframe='384').first(),
        TimeFrameData.query.filter_by(pair=pair, timeframe='768').first()
    ]
